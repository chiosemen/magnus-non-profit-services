/**
 * Parses ORG_IDENTITY markdown for GrantIntelligenceHerald (Candid matching).
 * Section headers must match templates (## Mission, ## Sector / NTEE, ## State footprint).
 */
...
function extractSection(md: string, header: string): string {
  const re = new RegExp(`^##\\s+${escapeRegex(header)}\\s*$`, 'mi');
  const m = re.exec(md);
  if (!m) return '';
