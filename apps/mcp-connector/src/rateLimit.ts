...
export const MCP_RATE_LIMIT_POINTS = 100;
export const MCP_RATE_LIMIT_DURATION_SECONDS = 60;
export const MCP_RATE_LIMIT_KEY_PREFIX = 'mcp_rl';
...
export async function buildMcpRateLimiter(options: BuildOptions = {}): Promise<RateLimiterLike> {
  ...
  if (!redisUrl) {
    if (isProduction) {
      throw new RateLimitBackendUnavailableError('REDIS_URL is required for production MCP rate limiting');
    }
    logger.warn('[magnus:mcp-rate-limit] REDIS_URL is not set. Using in-memory limiter for local dev/test only.');
    return new RateLimiterMemoryClass({
      keyPrefix: MCP_RATE_LIMIT_KEY_PREFIX,
      points: MCP_RATE_LIMIT_POINTS,
      duration: MCP_RATE_LIMIT_DURATION_SECONDS,
    });
  }
  ...
  try {
    ...
    logger.info('[magnus:mcp-rate-limit] Redis-backed rate limiter active (multi-instance safe).');
    return limiter;
  } catch (err) {
    if (isProduction) {
      throw new RateLimitBackendUnavailableError('Redis rate limit backend failed to connect');
    }
    logger.warn('[magnus:mcp-rate-limit] Redis connection failed. Falling back to in-memory limiter for local dev/test only.');
    return new RateLimiterMemoryClass({
      keyPrefix: MCP_RATE_LIMIT_KEY_PREFIX,
      points: MCP_RATE_LIMIT_POINTS,
      duration: MCP_RATE_LIMIT_DURATION_SECONDS,
...
export async function initializeMcpRateLimiter(): Promise<void> {
  rateLimiter = await buildMcpRateLimiter();
}

export async function getMcpRateLimiter(): Promise<RateLimiterLike> {
  if (!rateLimiter) rateLimiter = await buildMcpRateLimiter();
  return rateLimiter;
}

export function _resetMcpRateLimiterForTest(): void {
  rateLimiter = null;
}
